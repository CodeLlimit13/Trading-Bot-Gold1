...
// --- NEW: persistent primed flags
bool long_primed = false;   // true after M5 RSI <=30 while M30 trend up, stays true until RSI >=70 or trend flips
bool short_primed = false;  // true after M5 RSI >=70 while M30 trend down, stays true until RSI <=30 or trend flips
...
void OnTick()
{
    ...
    // --- Update primed flags (persistent until RSI flips or trend flips)
    double current_price = SymbolInfoDouble(_Symbol, SYMBOL_BID);
    double ema30 = ema_buffer[0];
    double rsi5 = rsi_buffer[0];
    
    // Prime LONG: M5 RSI <= 30 while M30 trend up (price > EMA)
    if(current_price > ema30 && rsi5 <= 30.0)
    {
        if(!long_primed)
        {
            long_primed = true;
            Print("Primed LONG: M5 RSI <= 30 while M30 trend up. (rsi=", DoubleToString(rsi5,2), " price=", DoubleToString(current_price,5), ")");
        }
    }
    // Un-prime LONG when RSI becomes overbought or trend flips
    if(long_primed && (rsi5 >= 70.0 || current_price <= ema30))
    {
        long_primed = false;
        Print("LONG primed cleared (RSI>=70 or 30m trend lost). (rsi=", DoubleToString(rsi5,2), " price=", DoubleToString(current_price,5), ")");
    }
    
    // Prime SHORT: M5 RSI >= 70 while M30 trend down (price < EMA)
    if(current_price < ema30 && rsi5 >= 70.0)
    {
        if(!short_primed)
        {
            short_primed = true;
            Print("Primed SHORT: M5 RSI >= 70 while M30 trend down. (rsi=", DoubleToString(rsi5,2), " price=", DoubleToString(current_price,5), ")");
        }
    }
    // Un-prime SHORT when RSI becomes oversold or trend flips
    if(short_primed && (rsi5 <= 30.0 || current_price >= ema30))
    {
        short_primed = false;
        Print("SHORT primed cleared (RSI<=30 or 30m trend lost). (rsi=", DoubleToString(rsi5,2), " price=", DoubleToString(current_price,5), ")");
    }
...
void CheckLongConditions()
{
    ...
    {
        // keep primed cleared if trend lost (safety)
        long_primed = false;
        return;
    }
    
    // require that RSI event already happened (RSI-first persistent primed)
    if(!long_primed) return; // not primed
    
    // now accept every bullish MACD crossover while still primed
    if(IsBullishMACDCrossover())
    {
        Print("Bullish MACD crossover detected while LONG primed -> placing long trades");
        PlaceLongTrades();
        // DO NOT clear long_primed here — we keep taking on every MACD crossover until RSI flips
    }
}
...
void CheckShortConditions()
{
    ...
    // require that RSI event already happened (RSI-first persistent primed)
    if(!short_primed) return; // not primed
    
    // now accept every bearish MACD crossover while still primed
    if(IsBearishMACDCrossover())
    {
        Print("Bearish MACD crossover detected while SHORT primed -> placing short trades");
        PlaceShortTrades();
        // DO NOT clear short_primed here — we keep taking on every MACD crossover until RSI flips
    }
}
